<?php
class ci_instituciones_vinculos extends gnosis_ci
{
    //-------------------------------------------------------------------------
    function relacion()
    {
    ...
    {
        $datos = toba::consulta_php('co_parametros')->get_instituciones_vinculos();
    ...
    function conf__form(gnosis_ei_formulario $form)
    {
        if ($this->relacion()->esta_cargada()) {
            $datos = $this->tabla('evt_instituciones_vinculos')->get();            
    ...
    {
        $this->tabla('evt_instituciones_vinculoss')->set($datos);
    ...
    {
        $this->tabla('evt_instituciones_vinculos')->set($datos);
        $this->relacion()->sincronizar();
        $this->relacion()->resetear();
    }
